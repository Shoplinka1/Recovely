...
function BillingPage() {
  ...
  {busy === p.id ? 'Opening checkout…' : state?.plan === p.id ? 'Current plan' : `Choose ${p.name}`}
            ...
            {!state?.paystackConfigured && <small className='billing-muted'>Checkout will report the real Paystack configuration status; no billing change is made until the server accepts it.</small>}
          </section>
        ))}
      </div>
